...
export default function Navbar() {
  ...
  return (
    ...
    <nav className="relative z-10 w-full bg-white md:absolute md:bg-transparent"style={{backgroundColor:"White"}}>
        ...
        <div className="flex flex-wrap items-center justify-between py-3 gap-6 md:py-4 md:gap-0">
          ...
          >
            ...
            <div className="w-full space-y-2 border-yellow-200 lg:space-y-0 md:w-max lg:border-l">
             {
              (user.length===0&&(localStorage.getItem('token')===''||localStorage.getItem('token')===undefined||localStorage.getItem('token')===null))?(
                <>
              ...
              >
                ...
                title="Start buying"
                className="w-full py-3 px-6 text-center rounded-full transition bg-yellow-300 hover:bg-yellow-100 active:bg-yellow-400 focus:bg-yellow-300 sm:w-max">
                   <span className="block text-yellow-900 font-semibold text-sm">Log Out</span>
                </button>
                </>
