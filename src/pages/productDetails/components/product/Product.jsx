...
const Product = ({id:productId}) => {
  ...
  const handleCart=(productId)=>{
    if(user.length===0&&(localStorage.getItem('token')===''||localStorage.getItem('token')===undefined||localStorage.getItem('token')===null)){
  ...
  <div className="container px-5 py-24 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap">
      ...
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        ...
        <p className="leading-relaxed"><span style={{color:"green"}}>Status</span>: {product?.productStatus}</p>
        <p className="leading-relaxed"><span style={{color:"green"}}>Stock Left </span>: {product?.productStockQty}</p>
        <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-200 mb-5">
         
        </div>
        <div className="flex">
          <span className="title-font font-medium text-2xl text-gray-900">NPR {product?.productPrice}</span>
          {product?.productStockQty>0&&status!=="loading"?(

            <button onClick={()=>handleCart(product?._id)} className=" flex ml-auto text-white bg-red-500 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded "disabled >Add To Cart </button>
            ):(
            <button onClick={()=>handleCart(product?._id)} className=" flex ml-auto text-white bg-red-300 border-0 py-2 px-6 focus:outline-none rounded " >Add To Cart </button>
